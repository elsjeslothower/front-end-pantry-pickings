...
const findByIngredientsRapidApi = async (req) => {
  try {
    const res = await axios.get(`${rapidApiUrl}/findByIngredients`, {
      params: {
...
const getRecipeInfoRapidApi = async (api_id) => {
  try {
    const res = await axios.get(`${rapidApiUrl}/${api_id}/information`, {
      params: {
        id: api_id,
      },
      headers: {
...
const RecipeCalculator = () => {
  ...
  const handleSaveRecipe = (
    ...
    getRecipeInfoRapidApi(api_id)
      .then((res) => {
        console.log(
          `handleSaveRecipe: success recipeInfoApi! ${res["id"]} here`
        );
        const newRecipeData = {
          api_id: api_id,
  ...
  return (
    <div className="container">
      ...
      <div
        ...
        role="alert"
      >
        <strong>Heads up!</strong> Links to recipes will only be active{" "}
        <strong>after</strong> you save them.
        <button
          type="button"
          className="btn-close"
